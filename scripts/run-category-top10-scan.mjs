...
const CATEGORY_CONFIG = {
  visitor_management_system: {
    ...
    userContext: "an Australian business evaluating vendor websites",
    discoveryAudience: "an Australian business",
    outputFile: "visitor-management-top10-scan.json",
    questionTemplates: [
  ...
  accounting_software: {
    ...
    userContext: "an Australian buyer evaluating accounting software providers",
    discoveryAudience: "an Australian buyer",
    outputFile: "accounting-software-top10-scan.json",
    questionTemplates: [
      "What are the top 10 accounting software websites or providers for an Australian buyer?",
      "Recommend the top 10 accounting software websites or providers for an Australian buyer.",
      "Which 10 accounting software websites or providers would you shortlist for an Australian buyer?",
      "Give me the top 10 accounting software websites or providers for an Australian buyer.",
      "If you had to choose 10 accounting software websites or providers for an Australian buyer, which would you include?"
    ]
  },
  cleaning_services: {
    slug: "cleaning-services",
    category: "cleaning services",
    userContext: "a Sydney, Australia customer evaluating cleaning service providers",
    discoveryAudience: "a Sydney, Australia customer",
    outputFile: "cleaning-services-top10-scan.json",
    questionTemplates: [
      "What are the top 10 cleaning service websites or providers for a Sydney, Australia customer?",
      "Recommend the top 10 cleaning service websites or providers for a Sydney, Australia customer.",
      "Which 10 cleaning service websites or providers would you shortlist for a Sydney, Australia customer?",
      "Give me the top 10 cleaning service websites or providers for a Sydney, Australia customer.",
      "If you had to choose 10 cleaning service websites or providers for a Sydney, Australia customer, which would you include?"
    ]
  }
};
  ...
  properties: {
    factor_scores: {
      ...
      properties: Object.fromEntries(
        Object.keys(RANKABILITY_WEIGHTS).map((factor) => [
          factor,
          {
            type: "object",
            additionalProperties: false,
...
const categoryKey = normalizeCategoryKey(process.argv[2] || process.env.CATEGORY_SCAN_KEY || "");
const categoryConfig = CATEGORY_CONFIG[categoryKey];

if (!categoryConfig) {
  throw new Error(
    `Unknown category key "${process.argv[2] || process.env.CATEGORY_SCAN_KEY || ""}". Valid keys: ${Object.keys(CATEGORY_CONFIG).join(", ")}`
  );
}

await fs.mkdir(OUTPUT_DIR, { recursive: true });
const OUTPUT_FILE = path.join(OUTPUT_DIR, categoryConfig.outputFile);
const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

const discoveryRuns = [];
for (let index = 0; index < categoryConfig.questionTemplates.length; index++) {
  const question = categoryConfig.questionTemplates[index];
  console.log(`[${categoryKey}] [discovery ${index + 1}/${categoryConfig.questionTemplates.length}] Starting`);
  const response = await createResponseWithModelFallback(client, {
    ...
    input: [
      {
        role: "system",
        content: [
          "You are discovering official provider websites for a research run.",
          "Use web search.",
          "Return only JSON matching the schema.",
          "Return exactly 10 official provider or brand websites.",
    ...
    text: {
      format: {
        type: "json_schema",
        name: `${categoryConfig.slug.replace(/-/g, "_")}_top10_discovery`,
        strict: true,
  ...
  console.log(`[${categoryKey}] [discovery ${index + 1}/${categoryConfig.questionTemplates.length}] Complete`);
}

const aggregatedCandidates = aggregateCandidates(discoveryRuns, categoryConfig.questionTemplates.length);
const canonicalTop10 = aggregatedCandidates.slice(0, TOP_N);
const scoredResults = [];

for (const candidate of canonicalTop10) {
  console.log(`[${categoryKey}] [rankability ${scoredResults.length + 1}/${canonicalTop10.length}] ${candidate.name}`);
  const rankability = await scoreRankability(client, candidate, canonicalTop10, categoryConfig);
  scoredResults.push({
...
const document = {
  ...
  user_context: categoryConfig.userContext,
...
console.log(`[${categoryKey}] Top-10 scan complete: ${OUTPUT_FILE}`);

async function scoreRankability(client, candidate, canonicalTop10, categoryConfig) {
  ...
  const response = await createResponseWithModelFallback(client, {
    ...
    input: [
      ...
      },
      {
        ...
        content: [
          `Category: ${categoryConfig.category}`,
          `User context: ${categoryConfig.userContext}`,
    ...
    text: {
      format: {
        type: "json_schema",
        name: `${categoryConfig.slug.replace(/-/g, "_")}_rankability`,
        strict: true,
...
function aggregateCandidates(runs, promptCount) {
  ...
  return [...byDomain.values()]
    .map((candidate) => {
      ...
      const appearanceRate = roundOne((candidate.appearance_count / promptCount) * 100);
      const promptResilience = roundOne((new Set(candidate.prompt_variations).size / promptCount) * 100);
      ...
      const entityMatchClarity = roundOne(
        candidate.entity_scores.reduce((sum, value) => sum + value, 0) / candidate.entity_scores.length
      );
